...
const Header = () => {
	...
	const navigation = [
		...
		...(!current_user
			? [
				{ name: 'Subscribe', href: '/register', current: false },

				{ name: 'Log In', href: '/login', current: false }
			]
			: []),
		{ name: 'About Us', href: '/about-us', current: false },
		{ name: 'Help Desk', href: '/contact-us', current: false }
	]
								...
								<div className="flex h-16 items-center justify-between">
									<div className="flex items-center justify-between w-full ">
										{/* <Link to="/" className="flex-shrink-0">
											<h2 className={`text-3xl tracking-wide font-bold hover:opacity-70 ${pathname === '/' && navbarClass === '' ? 'text-white' : 'text-blue-500'}`}>EarnByWatching</h2>
										</Link> */}
										<Link to="/" className="flex items-center justify-between w-25rem">
										{(pathname === '/' || pathname === '/about-us')&& navbarClass === '' ? <img className='logoimg' src={require('../../icons/logo-white.png')} alt="img" /> : <img className='logoimg2' src={require('../../icons/image.png')} alt="img" /> }
										</Link>
											...
											<div className="flex items-baseline space-x-4">
												{navigation.map((item) => (
													<Link
														...
														className={`px-3 py-2 rounded-md text-sm font-bold hover:opacity-70 ${(pathname === '/' || pathname === '/about-us') && navbarClass === '' ? 'text-white' : 'text-blue-500'}`}
